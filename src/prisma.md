...
8. You MUST ALWAYS use the full `pnpx prisma init` command with ALL flags: `pnpx prisma init --db --output ../src/generated/prisma --generator-provider prisma-client`
...
```bash
pnpx prisma init --db --output ../src/generated/prisma
```
...
pnpx prisma migrate dev --name X  # Create and apply migration
pnpx prisma studio                # Open visual database editor
pnpx prisma db push               # Push schema without migration
```
