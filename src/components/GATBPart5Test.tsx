...
export default function GATBPart5Test({
  ...
  useEffect(() => {
    ...
    if (Object.keys(responses).length > 0) {
      console.log(`📊 GATB Part 5 - Current Score: ${newScore.correct}/${newScore.total} (${newScore.percentage}%)`);
    }
  }, [responses, questions]);
  ...
  const handleSubmit = async (e?: React.FormEvent, forceSubmit = false) => {
    ...
    try {
      ...
      const globalHandler = typeof window !== 'undefined' ? (window as any).gatbPart5SubmitHandler : null;
      ...
      if (submitHandler) {
        ...
        if (result.score) {
          console.log('═══════════════════════════════════════');
          console.log('📊 GATB Part 5 - Test Results');
          console.log('═══════════════════════════════════════');
        ...
        window.location.href = '/tests/gatb-part-6-instructions';
      }
